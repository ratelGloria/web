...
$("#queryCategory").click(function(){
    var xmlHttpRequest = new XMLHttpRequest;
    xmlHttpRequest.open("get","/queryCategoryServlet?cate_parent_id=*",true);
    ...
    xmlHttpRequest.send();
     ...
     $("#shop_img").change(function () {
         ...
         var shop_img = document.getElementById("shop_img").files[0].name;
         var xmlHttpRequest = new XMLHttpRequest();



         xmlHttpRequest.open("get","/formatServlet?shop_img="+shop_img,true);
         xmlHttpRequest.onreadystatechange = function() {
             if (xmlHttpRequest.status === 200 && xmlHttpRequest.readyState === 4) {
                 var message = xmlHttpRequest.responseText;

                 var span = document.createElement("span")
                 var hintmessage = document.createTextNode(message)
                 span.append(hintmessage)
                 $("#hint").append(span)
             }
         }
         xmlHttpRequest.send()

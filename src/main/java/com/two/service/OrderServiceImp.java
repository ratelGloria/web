...
public class OrderServiceImp implements OrderService
{
    ...
    private BigDecimal getOrderPrice(List<OrderItem> orderItemList){
        ...
        for(OrderItem orderItem:orderItemList){
            ...
            bigDecimal = CalculateUtil.multiply(orderItem.getQuantity().doubleValue(),orderItem.getTotalPrice().doubleValue());
        }
        return bigDecimal;
    }
@Override
    public ServerResponse cartCreateOrder(Integer userId,Integer shippingId){
        List<Cart> cartList = cartMapper.findCartListByUserIdAndChecked(userId,Const.CartCheckEnum.PRODUCT_CHECKED.getCode());
    System.out.println("----------"+cartList);
        ServerResponse cartOrderItem = getCartOrderItem(userId,cartList);
        System.out.println("----------"+cartOrderItem.getStatus());
        if(cartOrderItem.getStatus() == 2){
            return ServerResponse.serverResponseByError("未选择");
        }

        BigDecimal orderTotalPrice = new BigDecimal("0");

        List<OrderItem> orderItemList = (List<OrderItem>) cartOrderItem.getData();
        System.out.println("pppppppppppppp"+orderItemList);
    ...
    @Override
    public ServerResponse orderInfo(Integer userId) {
    ...
    @Override
    public ServerResponse orderInfoLimit(Integer userId) {
    ...
    public ServerResponse orderDetail(Integer userId, Long orderNo) {
        ...
        if(orders.size()<=0){
            return ServerResponse.serverResponseByError("没找到订单");
        }

        List<OrderItem> orderItems1 = orderItemMapper.selectByUserIdAndOrderNo(userId, orderNo);

        Integer shippingId = orders.get(0).getShippingId();
        OrderVO orderVO = assembleOrderVO(orders.get(0), orderItems1, shippingId);
        //-----------------------------------------------------------------懒得写了，一会自己前面去调
        return ServerResponse.serverResponseBySuccess(orderVO);
    }
    ...
    public ServerResponse deleteOrder(Integer userId, Long orderNo) {
       ...
       /* int orderNo_int = orderNo;
        Long orderNo1 = new Long(orderNo_int);*/
        int i = orderMapper.deleteOrder(userId, orderNo);
        if(i>0){
...
*/
    ...
    public ServerResponse send_goods(Long orderNo) {
        ...
        if(order.getStatus() != Const.OrderStatusEnum.ORDER_SEND.getCode()){
            ...
            int i = orderMapper.updateByOrderNo(order);
            if(i>0){
                return ServerResponse.serverResponseBySuccess("已发货");
            }else {
    ...
    @Override
    public ServerResponse selectOrderPayStatus(Long orderNo) {

        if(orderNo == null){
            return ServerResponse.serverResponseByError("不能为空");
        }
        Order order = orderMapper.selectByOrderNo(orderNo);
        ...
        return ServerResponse.serverResponseByError("weichadao");
    }
  ...
  /*  private PayInfo getPayInfo(Order order){
    ...
    private OrderVO assembleOrderVO(Order order,List<OrderItem> orderItemList,Integer shipping){

        OrderVO orderVO = new OrderVO();

        List<OrderItemVoList> orderItemVoLists = Lists.newArrayList();
        for(OrderItem orderItem:orderItemList){
            OrderItemVoList orderItemVoList = assembleOrderItemVO(orderItem);
            orderItemVoLists.add(orderItemVoList);
        }
        orderVO.setOrderItemVoLists(orderItemVoLists);
        orderVO.setImageHost(PropertiesUtils.readByKey("imageHost"));
        Shipping shipping1 = shippingMapper.selectByPrimaryKey(shipping);
        if(shipping1 != null){
            orderVO.setShippingId(shipping);
            ShippingVO shippingVO = assbleShippingVO(shipping1);
            orderVO.setShippingVo(shippingVO);
            orderVO.setReceiverName(shipping1.getReceiverName());
        }
    ...
    private ShippingVO assbleShippingVO(Shipping shipping){
    ...
    private OrderItemVoList assembleOrderItemVO(OrderItem orderItem){
        OrderItemVoList orderItemVoList = new OrderItemVoList();

        if(orderItem != null){
            orderItemVoList.setQuantity(orderItem.getQuantity());
            orderItemVoList.setTotalPrice(orderItem.getTotalPrice());
            orderItemVoList.setProductImage(orderItem.getProductImage());
            orderItemVoList.setProductId(orderItem.getProductId());
            orderItemVoList.setOrderNo(orderItem.getOrderNo());
            orderItemVoList.setCurrentUnitPrice(orderItem.getCurrentUnitPrice());
            orderItemVoList.setCreateTime(orderItem.getCreateTime());
    ...
    //尽管调，不能下单算我输
    @Override
    public ServerResponse createOrder(Integer userId,Integer product, Integer shippingId) {

        if(product == null){
            return ServerResponse.serverResponseByError("请选则商品");
        }
        if(shippingId == null){
            return ServerResponse.serverResponseByError("请选择收货地址");
        }
        ...
        return ServerResponse.serverResponseBySuccess(getOrderItemVoList(product1));
    }

    @Override
    public ServerResponse createCartOrder(Integer userId, Integer shippingId) {
        if(shippingId == null){
            return ServerResponse.serverResponseByError("请选择收货地址");
        }
        List<Cart> cartListByUserIdAndChecked = cartMapper.findCartListByUserIdAndChecked(userId, Const.CartCheckEnum.PRODUCT_CHECKED.getCode());
        if(cartListByUserIdAndChecked.size()>0){
            ServerResponse cartOrderItem = getCartOrderItem(userId,cartListByUserIdAndChecked);
        }

        return ServerResponse.serverResponseByError("请选中商品");
    }

    private OrderVO getOrderItemVoList(Product product){

        OrderItemVoList orderItemVoList = new OrderItemVoList();
        OrderVO orderVO = new OrderVO();

        orderItemVoList.setCurrentUnitPrice(product.getPrice());
        orderItemVoList.setOrderNo(123456789L);
        orderItemVoList.setProductId(product.getId());
        orderItemVoList.setProductName(product.getName());
        orderItemVoList.setProductImage(product.getMainImage());
        orderItemVoList.setQuantity(1);
        orderItemVoList.setTotalPrice(CalculateUtil.multiply(product.getPrice(),1));


        orderVO.setOrderNo(orderItemVoList.getOrderNo());
        orderVO.setPayment(product.getPrice());
        orderVO.setPaymentType(1);
        orderVO.setPostage(0);
        ...
        orderVO.setShippingId(product.getId());
    ...
    private ServerResponse getCartOrderItem(Integer userId,List<Cart> cartList){
        ...
        List<OrderItem> orderIuems = new ArrayList();

        if(cartList.size()<=0){
            return ServerResponse.serverResponseByError("kong");
        }
            for(int i=0;i<cartList.size();i++){
                Product product = productMapper.selectByPrimaryKey(cartList.get(i).getId());
                OrderItem orderItem = new OrderItem();
                orderItem.setCreateTime(null);
                orderItem.setCurrentUnitPrice(product.getPrice());
                orderItem.setOrderNo(123456789123L);
                orderItem.setProductId(cartList.get(i).getProductId());

                orderItem.setProductImage(product.getMainImage());
                orderItem.setUserId(userId);
                orderItem.setQuantity(cartList.get(i).getQuantity());
                orderItem.setTotalPrice(CalculateUtil.multiply(product.getPrice(),cartList.get(i).getQuantity()));
                System.out.println("111111111111++++++"+orderItem.getTotalPrice()+"------------"+cartList.get(i).getQuantity());
                orderItem.setUpdateTime(null);
                orderItem.setProductName(product.getName());
                orderIuems.add(orderItem);
            }
            return ServerResponse.serverResponseBySuccess(orderIuems);

       /* return ServerResponse.serverResponseByError("空");*/
    }
    ...
    private static AlipayMonitorService monitorService;
     ...
     static  {
        ...
        System.out.println("读到了么？？？？？");
         System.out.println("读到了么==============="+tradeService);
        /** 使用Configs提供的默认参数
    ...
    public ServerResponse pay(Order order) {
      ...
      /*  // 创建一个商品信息，参数含义分别为商品id（使用国标）、名称、单价（单位为分）、数量，如果需要添加商品类别，详见GoodsDetail
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl("http://39.96.43.223:8888/order/callback")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
